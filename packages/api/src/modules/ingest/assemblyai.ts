...
export type TranscribeAudioOptions = {
  /**
   * Enables server-side URL fetch fallback for trusted sources (for example,
   * signed storage URLs generated by this backend).
   */
  allowRemoteFetchFallback?: boolean;
  /**
   * Called periodically during long-running AssemblyAI polling to keep
   * the asset row's `updatedAt` fresh and avoid stale-asset recovery.
   */
...
/** Interval between heartbeat calls during long polls (5 minutes). */
...
async function transcribeViaUrl(
  audioUrl: string,
  onHeartbeat?: () => Promise<void>,
): Promise<TranscriptionResult> {
  const transcriptId = await submitTranscript(audioUrl);
  const transcript = await pollTranscript(transcriptId, onHeartbeat);
...
export async function transcribeAudio(
  ...
  try {
    return await transcribeViaUrl(audioUrl, options?.onHeartbeat);
  } catch (error) {
    const message =
      error instanceof Error ? error.message : 'Unknown transcription error';

    if (
      !options?.allowRemoteFetchFallback ||
      !shouldRetryViaUpload(audioUrl, message)
    ) {
      throw error;
    }

    const uploadUrl = await uploadRemoteAudio(audioUrl);
    return transcribeViaUrl(uploadUrl, options?.onHeartbeat);
  }
}
...
export async function transcribeAudioStream(
  ...
  return transcribeViaUrl(uploadUrl, onHeartbeat);
}
